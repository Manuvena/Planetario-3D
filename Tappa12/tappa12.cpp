...
public:
    ...
    //costruttore orbita con raggio (distanza dal sole) e numero di segmenti del cerchio che approssimo con 120 segmenti
...
public:
    ...
    GPUStars(int count=1200) {
        ...
        for(int i=0; i<count; ++i) {
            ...
            float r=60.0f+static_cast<float>(rand())/RAND_MAX*40.0f; //distanza della stella dal centro tra 60 e 100
...
public:
    ...
    float aspect=1.0f, phi=30.0f, theta=30.0f, fd=2.0f, od=25.0f; //parametri della camera
    ...
    void view_projection() { //ricalcola vista e proiezione
        ...
        float ncp=std::max(0.1f, od-20.0f), fcp=150.0f; //piani di clipping near e far
        float a=(fcp+ncp)/(ncp-fcp), b=2.0f*fcp*ncp/(ncp-fcp); //coefficienti della matrice di proiezione prospettica
...
public:
    ...
    GLuint t_sole, t_merc, t_ven, t_ter, t_luna, t_mar, t_gio, t_sat, t_ura, t_net; //id restituiti da caricaTexture
    ...
    void draw(float t) {
        ...
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &pos_sole[0][0]); //invia pos_sole a uniform model nella posizone m_loc, è una matrice e non viene trasposta
        stars.draw(); ///disegno le stelle
        glUniform1i(is_star_loc, 0); //disattivo modalità stelle

        //orbite geometriche
        glUniform1i(is_orbit_loc, 1);
        glUniform4f(color_loc, 0.2f, 0.25f, 0.35f, 1.0f); //imposto uniform del colore: rgb e alpha per opacità
        ...
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &c_ter[0][0]); //passo c_ter come matrice model, e disegno orbita luna che sarà cetnrata nella terra
        o_luna.draw();
        ...
        glm::mat4 m_sole=pos_sole*rotation_y(t*10.0f)*scaling(1.5f)*mesh_base_norm;
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &m_sole[0][0]);
        glUniform1i(is_sun_loc, 1);
        glBindTexture(GL_TEXTURE_2D, t_sole);
        mesh.draw();

        glm::mat4 m_merc=pos_sole*rotation_y(t*90.0f)*translation(2.0f, 0.0f, 0.0f)*rotation_y(t*40.0f)*scaling(0.24f)*mesh_base_norm;
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &m_merc[0][0]);
        glUniform1i(is_sun_loc, 0);
        glBindTexture(GL_TEXTURE_2D, t_merc);
        mesh.draw();

        glm::mat4 m_ven=pos_sole*rotation_y(t*60.0f)*translation(3.2f, 0.0f, 0.0f)*rotation_y(-t*20.0f)*scaling(0.34f)*mesh_base_norm;
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &m_ven[0][0]);
        glUniform1i(is_sun_loc, 0);
        glBindTexture(GL_TEXTURE_2D, t_ven);
        mesh.draw();

        glm::mat4 m_ter=c_ter*rotation_y(t*80.0f)*scaling(0.38f)*mesh_base_norm;
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &m_ter[0][0]);
        glUniform1i(is_sun_loc, 0);
        glBindTexture(GL_TEXTURE_2D, t_ter);
        mesh.draw();

        glm::mat4 m_luna=c_ter*rotation_y(t*110.0f)*translation(0.7f, 0.0f, 0.0f)*scaling(0.13f)*mesh_base_norm;
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &m_luna[0][0]);
        glUniform1i(is_sun_loc, 0);
        glBindTexture(GL_TEXTURE_2D, t_luna);
        mesh.draw();

        glm::mat4 m_mar=pos_sole*rotation_y(t*35.0f)*translation(6.2f, 0.0f, 0.0f)*rotation_y(t*70.0f)*scaling(0.28f)*mesh_base_norm;
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &m_mar[0][0]);
        glUniform1i(is_sun_loc, 0);
        glBindTexture(GL_TEXTURE_2D, t_mar);
        mesh.draw();

        glm::mat4 m_gio=pos_sole*rotation_y(t*18.0f)*translation(8.2f, 0.0f, 0.0f)*rotation_y(t*120.0f)*scaling(0.68f)*mesh_base_norm;
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &m_gio[0][0]);
        glUniform1i(is_sun_loc, 0);
        glBindTexture(GL_TEXTURE_2D, t_gio);
        mesh.draw();

        glm::mat4 m_sat=pos_sole*rotation_y(t*12.0f)*translation(10.5f, 0.0f, 0.0f)*rotation_y(t*100.0f)*scaling(0.58f)*mesh_base_norm;
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &m_sat[0][0]);
        glUniform1i(is_sun_loc, 0);
        glBindTexture(GL_TEXTURE_2D, t_sat);
        mesh.draw();

        glm::mat4 m_ura=pos_sole*rotation_y(t*8.0f)*translation(12.8f, 0.0f, 0.0f)*rotation_y(-t*60.0f)*scaling(0.46f)*mesh_base_norm;
        glUniformMatrix4fv(m_loc, 1, GL_FALSE, &m_ura[0][0]);
        glUniform1i(is_sun_loc, 0);
        glBindTexture(GL_TEXTURE_2D, t_ura);
        mesh.draw();

        glm::mat4 m_net=pos_sole*rotation_y(t*5.0f)*translation(15.0f, 0.0f, 0.0f)*rotation_y(t*50.0f)*scaling(0.43f)*mesh_base_norm;
...
int main(int argc, char** argv) {
    ...
    Scene sc("../Risorse/sfera.off", sh, *s.window); //costruttore scena
    ...
    while(s.window->isOpen()) { //ciclo ripetuto finchè la finestra è aperta
        ...
        glClearColor(0.01f, 0.01f, 0.02f, 1.0f); //colore di pulizia per lo sfondo
